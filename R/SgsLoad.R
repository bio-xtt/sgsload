...
#' }
#'
gain_signac <- function( # track_type,
                        object,
                        assays,
                        matrix.slot,
                        assay.type,
  ...
  ## gain the merge file
  merged_file <- cbind(feature = rownames(metadata), metadata)
  ...
  for (embedding in reducNames) {
    ...
    merged_file <- merge(merged_file, df, by = "feature")
  }
  ...
  if (!is.null(assays)) {
    ...
    for (i in assays) {
      ...
      exp_df_t <- cbind(feature = rownames(counts_t), counts_t)
      rownames(exp_df_t) <- NULL
      all_file <- merged_file
      all_file <- merge(exp_df_t, all_file, by = "feature", all = T)
      ...
      feature_names <- tolower(rownames(counts))
      ...
      exp_info <- list("matrix" = feaher_path, "feature_names" = feature_names, "marker" = marker_path)
      ...
      if (assay.type[i][[1]] == "gene") {
        ...
        if (!is.null(marker.files[i][[1]]) && length(GetMotifData(object = object, slot = "pwm")) > 0) {
          ...
          all_exp_info$motif_pfm <- pwm_file_path
        } else {
        ...
        if (length(Links(object = object)) > 0) {
          ...
          all_exp_info$co_access <- co_file_path
        } else {
...
ExportSC <- function(object,
  ...
  #
  # ####gain post url
  # if(track_type == "sc_RNA"){
  #   post_url <- "......"
  #
  # }else if(track_type == "sc_ATAC"){
  #   post_url <- "......."
  # }
  #
